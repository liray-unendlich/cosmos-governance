...
export default class LcdClient {
	...
	async getProposalsV1() {
		...
		return filteredProposals.map((proposal) => ({
			...
			title: proposal?.messages[0]?.title || proposal?.messages[0]?.content?.title || proposal?.title,
			description: proposal?.messages[0]?.description || proposal?.messages[0]?.content?.description || proposal?.summary,
			status: proposal?.status,
			plan: proposal?.messages[0]?.content?.plan || proposal?.messages[0]?.plan,
			submitTime: new Date(proposal.submit_time),
			votingStartTime: new Date(proposal.voting_start_time),
			votingEndTime: new Date(proposal.voting_end_time),
		}));
